...
pub fn generate_token(
    ...
    match paseto::tokens::PasetoBuilder::new()
        ...
        .set_issuer("qreeket-qcodelabsllc")
        .set_audience("authenticated-users")
        .set_jti("qreeket-qcodelabsllc")
...
pub fn generate_public_token(language_id: &str) -> Result<String, Box<dyn Error>> {
    // calculate token expiration in milliseconds from current time + 60 minutes ttl using chrono
    let ttl = 3600;
    let token_ttl = _calculate_token_expiration(ttl);
    ...
    match paseto::tokens::PasetoBuilder::new()
        ...
        .set_claim(
            "go-to",
            json!("https://github.com/quabynah-bilson/qreeket/tree/dev/mobile"),
        )
        .set_footer(format!("key-id:{}", &auth_token.clone()).as_str())
        .build()
    {
...
// generates a reset token to reset user password.
pub fn generate_reset_token(language_id: &str) -> Result<String, Box<dyn Error>> {
    // calculate token expiration in milliseconds from current time + 10 minutes ttl using chrono
    let ttl = 600;
    let token_ttl = _calculate_token_expiration(ttl);

    // create reset token
    let auth_token = std::env::var("PUB_RESET_PWD_TOKEN").expect("PUB_RESET_PWD_TOKEN must be set");
    match paseto::tokens::PasetoBuilder::new()
        .set_encryption_key(&*auth_token.to_owned().into_bytes())
        .set_issued_at(Some(Utc::now()))
        .set_expiration(&Utc.timestamp_opt(token_ttl, 0).single().unwrap())
        .set_issuer("qreeket-qcodelabsllc")
        .set_audience("reset-password")
        .set_jti("qreeket-qcodelabsllc")
        .set_not_before(&Utc::now())
        .set_subject("password-reset-only")
        .set_claim(
            "go-to",
            json!("https://github.com/quabynah-bilson/qreeket/tree/dev/mobile"),
        )
...
pub async fn validate_reset_token(token: &str, language_id: &str) -> Result<(), Box<dyn Error>> {
    // check if token is expired and return error if it is
    match _check_reset_token_expiration(&token, &language_id).await {
        Ok(_) => Ok(()),
        Err(e) => Err(e),
    }
}
...
pub async fn validate_token(
    ...
    if has_expired {
        ...
        let token_store = match token_col
...
async fn _check_public_token_expiration(
    token: &str,
    language_id: &str,
...
async fn _check_reset_token_expiration(
    token: &str,
    language_id: &str,
) -> Result<(), Box<dyn Error>> {
    // decrypt token
...
pub(crate) fn get_payload_from_token(token: &str) -> Result<(String, String), Box<dyn Error>> {
    ...
    // decrypt token
    let payload = match decrypt_paseto(
        &token,
        Some(format!("key-id:{}", &auth_token).as_str()),
        &mut &auth_token.into_bytes(),
    ) {
        Ok(payload) => payload,
        Err(_) => {
            return Err(Box::new(std::io::Error::new(
    ...
    let account_id = match json["qreeket-owner"]["account_id"].as_str() {
    ...
    let language_id = match json["qreeket-owner"]["language_id"].as_str() {
        Some(id) => id.to_string(),
        None => {
            return Err(Box::new(std::io::Error::new(
                ErrorKind::Other,
